...
b_gender <- 'f'
b_race   <- 'black'
racedata <- tripletdata[gender == b_gender & race == b_race]
raceids  <- tripletmeta[gender == b_gender & race == b_race]
setorder(raceids, -count)
#raceids  <- raceids[, .SD[1:270]] # BM
raceids  <- raceids[, .SD[1:90]] # BF & AF
#raceids  <- raceids[, .SD[1:180]] # IF 
racedata <- racedata[id %in% raceids$id]
setorder(racedata, -count)
min(racedata$count)
#racedata <- racedata[, .SD[sample.int(count, 128)], by = 'id']
...
#racedata <- racedata[, id_bin := cut(id_index, breaks = seq(0, 180, 90), right = T, labels = F)]
#table(racedata$id_bin)
#racedata$id_bin <- racedata$id_bin + 1
racedata$id_bin <- 1
racedata <- racedata[, img_index := seq_len(.N), by = 'id']
setorder(racedata, id_bin, img_index)

set1 <- racedata
set2 <- racedata
set3 <- racedata
racedata <- rbind(set3, set1, set2)
table(racedata$id_bin)

racedata <- racedata[, batch_bin := cut(img_index, breaks = seq(0, max(racedata$img_index), 8), right = T, labels = F)]
setorder(racedata, batch_bin, id_bin, img_index, id_index)
table(racedata$gender, racedata$batch_bin)

racedata$source <- paste0('/Users/santhoshnarayanan/Documents/Turing/GPU/VGGData/aligned/images/',
                          racedata$id, '/', racedata$filename)

writedf <- racedata[, c('id', 'source')]
writedf$id <- as.integer(as.factor(writedf$id))
write.table(writedf, 
            file = '/Users/santhoshnarayanan/Documents/Turing/GPU/BiasTests/OnlineTriplets/Data/BF_images_local.csv', 
            sep = ',', row.names = F)

#### Paperspace write
racedata$dest_file <- paste0(racedata$id, '_', racedata$filename)
racedata$destination <- paste0('/Users/santhoshnarayanan/Documents/Turing/GPU/VGGData/BFtrain')
...
path  <- paste0('/Users/santhoshnarayanan/Documents/Turing/GPU/BiasTests/OnlineTriplets/convnet/rep_BF.csv')
...
pdf(paste0("CONVNET_BF.pdf"), height=3, width=2.2)
grid.table(results)
dev.off()
